...
func (t *Trie) Filter(text string) (target string) {
	root := t.Root
	rs := []rune(text)
	rs_len := len(rs)
	var offset int
	t_location := location{}
	for i := 0; i < rs_len; {
		if r, ok := root.Node[rs[offset+i]]; ok {
			root = r
			offset += 1
			var exist bool = false
			if offset+i < rs_len {
				_, exist = root.Node[rs[offset+i]]
			}
			if root.End && !exist {
				//	fmt.Println("root.End:", string(rs[i:i+offset]), " start:", i, " end:", i+offset)
				if offset > t_location.len {
					t_location.loc = i
					t_location.len = offset
				}
				if offset == t_location.len && i > t_location.loc {
					t_location.loc = i
				}
				i = i + offset
				offset = 0
	...
	target = string(rs[t_location.loc : t_location.loc+t_location.len])
	return
}
